...
static int scull_u_open(struct inode *inode, struct file *filp)
{
	...
	if (scull_u_count && 
			(scull_u_owner != current->real_cred->uid) &&  /* allow user */
			(scull_u_owner != current->real_cred->euid) && /* allow whoever did su */
...
static inline int scull_w_available(void)
{
	return scull_w_count == 0 ||
		scull_w_owner == current->real_cred->uid ||
		scull_w_owner == current->real_cred->euid ||
